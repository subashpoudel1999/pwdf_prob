  ...
  for (int i = 0; i < rawFeatures.length; i++) {
    ...
    for (final polygon in polygonCoords) {
      ...
      for (final ring in polygon) {
        final points = <LatLng>[];
        bool ringHasUtm = false;
        for (final coord in ring) {
          final lng = (coord[0] as num).toDouble();
          final lat = (coord[1] as num).toDouble();
          // Skip coordinates that are clearly in a projected CRS (UTM metres)
          // rather than geographic degrees. This prevents flutter_map assertions.
        ...
        if (!ringHasUtm) rings.add(points);
      }
